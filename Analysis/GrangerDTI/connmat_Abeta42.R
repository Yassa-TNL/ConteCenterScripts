...
inputPath <- "/Volumes/yassamri/ADRC/ProcessedData/Soyun_Steve_Analysis/RJ_TESTING/Input/n45_total"
covaPath <- "/Volumes/yassamri/ADRC/ProcessedData/Soyun_Steve_Analysis/RJ_TESTING/Input/abeta42.csv"

covsFormula <- "~Abeta42"
ChangeType<-list("as.numeric")
CorrType="pearson"

OutDirRoot <- "/Volumes/yassamri/ADRC/ProcessedData/Soyun_Steve_Analysis/RJ_TESTING/Output"
SubOutDir="DMN"

VarsofInterest<-c("Frontal_Mid_Orb_L", "Frontal_Mid_Orb_L", "Cingulum_Ant_L", "Cingulum_Ant_R")
...
for (Subject in 1:MaxSubject){
	...
	if (!is.null(VarsofInterest) && length(VarsofInterest) > 0){
		VarNumInterest<-which(ColumnNAMES[[1]] %in% VarsofInterest)
...
write.csv(Rmatrix, paste(OutSubDir,"/n",dim(DATASET)[1],"_R-Matrix-",CorrType,"_",FileName,"_",Date,".csv", sep=''))
