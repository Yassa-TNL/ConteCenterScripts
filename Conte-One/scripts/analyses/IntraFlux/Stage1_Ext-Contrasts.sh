...
module purge ; module load anaconda/2.7-4.3.1 singularity/3.3.0 R/3.5.3 fsl/6.0.1 mrtrix/3.0_RC3
source ~/Settings/MyPassCodes.sh
source ~/Settings/MyCondaEnv.sh
conda activate local

#######################################################
### Resample Yeo 7-ROI Atlas & Group ICA Components ###
#######################################################
...
for NETWORK in `ls ${DIR_PROJECT}/apps/xcp-fcon/pipe-aroma_task-AMG/group/n138_IntraFlux.gica/dim-10_sm3/smooths_final/*` ; do 
	OUTFILE=`echo ${DIR_TOOLBOX}/bids_apps/atlases/$(basename $NETWORK | sed s@probmap@atl-gica@g | sed s@smth-7_@network-@g)`
	mrresize -size 91,109,91 ${NETWORK} ${OUTFILE}
	chmod 740 $OUTFILE
done
