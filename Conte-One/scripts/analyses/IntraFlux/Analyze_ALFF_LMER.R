...
covaPath <- "/dfs2/yassalab/rjirsara/ConteCenterScripts/Conte-One/analyses/IntraFlux/n138_IntraFlux.mods/ALFF.csv" 
...
DATASETS<-DATASETS[grepl("ALFF_Longitudinal.csv",DATASETS)]
...
########################################################
	...
	m <- mclapply(model.formula, function(x) {
		ANALYZE <- lmer(formula = x, random=as.formula(randomFormula), data=dataSubj)
