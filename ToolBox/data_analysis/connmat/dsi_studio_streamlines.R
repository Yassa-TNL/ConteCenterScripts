...
if( missing("MATRICES") | missing("IN_CSV") | missing("DIR_OUT") | missing("FORMULA")){
	...
	cat(" Usage:",'\n')
	cat('\n')
	cat("This Script Takes in Raw Subject-Level Matrices of Steamlines that were generated by DSI-Studio ")
	...
	cat('\n')
	cat("MATRICES: Path to where the subject-level matrices (DSI-Studio Output)",'\n')
...
InputFiles = list.files(path= MATRICES,pattern="*.txt", full.names = TRUE)
...
if (CORR_TYPE == "pearson" || CORR_TYPE == "kendall" || CORR_TYPE == "Spearman"){
	print(paste("#######################################"))
	print(paste(CORR_TYPE,"Correlations Will Be Executed "))
	print(paste("#######################################"))
} else {
	print(paste("⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! ⚡ ! "))
...
if (MaxPredictors == 1){
	...
	for (connection in 1:dim(SPREADSHEET)[2]){
		...
		ConnR<-suppressWarnings(cor.test(DATASET[,MainPredict],DATASET[,ConnNum], method=CORR_TYPE)$estimate)
		ConnP<-suppressWarnings(cor.test(DATASET[,MainPredict],DATASET[,ConnNum], method=CORR_TYPE)$p.value)
		if (is.null(ConnR)){
			ConnR<-NA
		}
...
RESP<-basename(MATRICES)
COVA<-basename(IN_CSV)
COVA<-gsub(".csv","",COVA)

DataSubDir<-paste0(DIR_OUT,"/Data/COVA-",COVA,"_RESP-",RESP,"/",DIR_SUBOUT,split="")
DataSubDir<-gsub(' ','',DataSubDir)
dir.create(file.path(DataSubDir), showWarnings = FALSE, recursive = TRUE, mode = "0775")

FigSubDir<-paste0(DIR_OUT,"/Figures/COVA-",COVA,"_RESP-",RESP,"/",DIR_SUBOUT,split="")
FigSubDir<-gsub(' ','',FigSubDir)
dir.create(file.path(FigSubDir), showWarnings = FALSE, recursive = TRUE, mode = "0775")

OutSubDir<-paste0(DIR_OUT,"/Results/COVA-",COVA,"_RESP-",RESP,"/",DIR_SUBOUT,split="")
...
write.csv(Rmatrix, paste(OutSubDir,"/n",dim(DATASET)[1],"_R-Matrix-",CORR_TYPE,"_",FileName,"_",Date,".csv", sep=''))
...
pdf(paste(FigSubDir,"/n",dim(DATASET)[1],"_R-Matrix_",CORR_TYPE,"_",FileName,"_",Date,".pdf", sep=''),width=6,height=5,paper='special')
levelplot(Rmatrix)
dev.off()

pdf(paste(FigSubDir,"/n",dim(DATASET)[1],"_P-Matrix_",CORR_TYPE,"_",FileName,"_",Date,".pdf", sep=''),width=6,height=5,paper='special')
levelplot(Pmatrix)
dev.off()
