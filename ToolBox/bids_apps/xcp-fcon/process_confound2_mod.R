...
DIR_DATASETS=unique(dirname(list.files(path=DIR_LOCAL_DATA, full.names=T, recursive=T, pattern = "_QA-Summary_")))
for (DIR_TASK in DIR_DATASETS[grep("prestats",DIR_DATASETS)]){
	LABEL_TASK=gsub("func-","",basename(gsub("prestats","",DIR_TASK)))
...
################################################
	...
	QADATA<-as.data.frame(CONTENT[,c(grep("DOFLOST",names(CONTENT)))])
	names(QADATA)<-names(CONTENT)[grep("DOFLOST", names(CONTENT))]
...
for (THRFILE in THRFILES){
	print(paste0("Reorganizing The Threshold Dataset From: ",THRFILE))
