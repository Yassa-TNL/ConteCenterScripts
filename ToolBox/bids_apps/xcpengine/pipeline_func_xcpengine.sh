	...
	for SCAN in ${SCANS} ; do
		if [[ $HEADER_FORMAT == "Standard" ]] ; then
			...
			if [[ $SCAN == *"run-"* ]] ; then
				...
				echo "sub-${SUB},task-${TASK}_run-${RUN},${SCAN}" >> ${COHORT_FILE}
			else
				SUB=`basename $SCAN | cut -d '_' -f1 | cut -d '-' -f2`
				echo "sub-${SUB},task-${TASK}_run-01,${SCAN}" >> ${COHORT_FILE}
			fi
			...
			if [[ $SCAN == *"run-"*  ]] ; then
				...
				echo "sub-${SUB},ses-${SES},task-${TASK}_run-${RUN},${SCAN}" >> ${COHORT_FILE}
			else
				SUB=`basename $SCAN | cut -d '_' -f1 | cut -d '-' -f2`
 				SES=`basename $SCAN | cut -d '_' -f2 | cut -d '-' -f2`
				echo "sub-${SUB},ses-${SES},task-${TASK}_run-01,${SCAN}" >> ${COHORT_FILE}
			fi
		fi
	done
...
##################################
	...
	rmdir ${DIR_OUTPUT_PATH}/workflows/single_subject_${SUBJECT}

###################################################################################
### Extract Signal From Tissue Segmentations Following Demeaning and Detrending ###
###################################################################################

	for SEGMENT_NIFTI in `find ${DIR_OUTPUT_PATH}/sub-${SUBJECT} -type f | grep task-${TASK} | grep prestats | grep segmentation.nii.gz` ; do
...
##########################################################
	...
	if [[ $HEADER_FORMAT == *"MultiRun"* ]] ; then
		for DIR_ROOT in `find ${DIR_OUTPUT_PATH}/sub-${SUBJECT} -type d | grep task-${TASK} | sed s@'task-'@'#'@g | cut -d '#' -f1| cut -d ',' -f1 | uniq` ; do
			echo 'Merging Selected Files From Fcon Module'
			for ATLAS in `ls $(ls -d ${DIR_ROOT}/task-${TASK}_run-*/fcon  | tr '\n' ' ' | cut -d ' ' -f1) | grep -v .nii.gz` ; do
				mkdir -p ${DIR_ROOT}/task-${TASK}_concat/fcon/${ATLAS}/
				MERGE=`echo ${DIR_ROOT}/task-${TASK}_run-*/fcon/${ATLAS}/sub-${SUBJECT}*task-${TASK}_run-*_${ATLAS}_ts.1D`	
				MERGED_FILE=`echo $MERGE | cut -d ' ' -f1 | sed s@run-01@'concat'@g` ; cat $MERGE > $MERGED_FILE
				singularity exec --cleanenv ${DIR_LOCAL_SCRIPTS}/container_xcpengine.simg \
