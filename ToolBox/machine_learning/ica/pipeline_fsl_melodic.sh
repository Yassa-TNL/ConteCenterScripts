...
module purge ; module load fsl/6.0.1
rm GroupICA.*
...
##########################################################
	...
	for EXTENSION in `echo $SCANS_PREPROC | tr ' ' '\n' | sed s@residualised@'#'@g | cut -d '#' -f2 | sort | uniq | grep _sm` ; do
		if [[ -f ${OPT_INCLUSION_FILE} && ! -z `cat $OPT_INCLUSION_FILE | grep $OPT_INCLUSION_VAR` ]] ; then
			if [[ $(echo $SCANS_PREPROC | tr ' ' '_') != *"ses-"* ]] ; then
				...
				for ROW in `cat ${OPT_INCLUSION_FILE} |  csvcut -c sub,ses,${OPT_INCLUSION_VAR} | grep 1$` ; do
					...
					SCAN=`echo $SCANS_PREPROC | tr ' ' '\n' | grep "/sub-${SUB}_ses-${SES}_*residualised${EXTENSION}"`
					sed -i "s@^${ROW}@${SCAN}@g" cohort.csv > /dev/null 2>&1
				done
			else
				exit 0
			...
			if [[ ! -z $OPT_INCLUSION_VAR && ${OPT_INCLUSION_VAR} == *'_Inclusion' ]] ; then
				FILE_COHORT=$DIR_PIPE/group/n$(cat cohort_FINAL.csv | wc -l)_$(echo $OPT_INCLUSION_VAR | cut -d '_' -f1).gica/cohort$(echo $EXTENSION | cut -d '.' -f1)_${TODAY}.csv
			else
				FILE_COHORT=$DIR_PIPE/group/n$(cat cohort_FINAL.csv | wc -l)_$(basename $DIR_PROJECT | cut -d '/' -f1)_subsample.gica/cohort$(echo $EXTENSION | cut -d '.' -f1)_${TODAY}.csv
			fi
			mkdir -p $(dirname $FILE_COHORT)
			mv cohort_FINAL.csv $FILE_COHORT ; rm cohort.csv
		else
			FILE_COHORT=$DIR_PIPE/group/n$(echo $SCANS_PREPROC | wc -w)_$(basename $DIR_PROJECT | cut -d '/' -f1)_fullsample.gica/cohort$(echo $EXTENSION | cut -d '.' -f1)_${TODAY}.csv
			echo $SCANS_PREPROC | tr ' ' '\n' | grep residualised${EXTENSION} > $FILE_COHORT
		fi
...
###############################################################################
		...
		for NCOMP in $NCOMP_ITERATIONS ; do
			DIR_OUTPUT=$(echo $(dirname $FILE_COHORT)/dim-${NCOMP}$(echo $EXTENSION | cut -d '.' -f1))
			mkdir -p ${DIR_OUTPUT}
			...
			${GRPICA_COMMAND} > `echo $GRPICA_COMMAND | sed s@"command"@"dim-${NCOMP}$(echo $EXTENSION | cut -d '.' -f1)/log"@g | sed s@'.sh'@'.txt'@g` 2>&1

			rm `echo $(dirname $FILE_COHORT)/dim-${NCOMP}$(echo $EXTENSION | cut -d '.' -f1)/log.txt`
			mkdir -p $(echo $(dirname $FILE_COHORT)/dim-${NCOMP}$(echo $EXTENSION | cut -d '.' -f1)/smooths)
			for MAP in `find $DIR_OUTPUT | grep 'stats/probmap'` ; do
				fslmaths $MAP -s 2 -thr 0.35 $(echo $MAP | sed s@"probmap_"@"probmap_smth-2_"@g | sed s@'stats'@'smooths'@g)
				fslmaths $MAP -s 3 -thr 0.35 $(echo $MAP | sed s@"probmap_"@"probmap_smth-3_"@g | sed s@'stats'@'smooths'@g)
				fslmaths $MAP -s 4 -thr 0.35 $(echo $MAP | sed s@"probmap_"@"probmap_smth-4_"@g | sed s@'stats'@'smooths'@g)
				fslmaths $MAP -s 5 -thr 0.35 $(echo $MAP | sed s@"probmap_"@"probmap_smth-5_"@g | sed s@'stats'@'smooths'@g)
			done

			if [[ ${OPT_DUAL_REGRESS} == "TRUE" ]] ; then

				NIFTI_GRPICA_OUTPUT=$(echo `dirname $GRPICA_COMMAND`/dim-${NCOMP}$(echo $EXTENSION | cut -d '.' -f1)/melodic_IC.nii.gz)
				DIR_DUALREG_OUTPUT=$(echo `dirname $FILE_COHORT | sed s@.gica@.dreg@g`/dim-${NCOMP}$(echo $EXTENSION | cut -d '.' -f1))
				...
				${DUALREG_COMMAND} > `echo $DUALREG_COMMAND | sed s@"command"@"dim-${NCOMP}$(echo $EXTENSION | cut -d '.' -f1)/log"@g | sed s@'.sh'@'.txt'@g` 2>&1

				module load anaconda/2.7-4.3.1 singularity/3.3.0

			fi
			chmod -R ug+wrx `echo $(dirname $FILE_COHORT)/dim-${NCOMP}$(echo $EXTENSION | cut -d '.' -f1)`	
		done
	done
done
