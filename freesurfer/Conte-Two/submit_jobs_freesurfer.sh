...
AllSubs=`ls -d1 /dfs2/yassalab/rjirsara/ConteCenter/BIDs/Conte-Two/sub-*/ses-*/anat/*T1w.nii.gz | cut -d '/' -f8,9 | cut -d '-' -f2,3 | sed s@'/ses-'@','@g`

for subject in ${AllSubs} ; do

  sub=`echo $subject | cut -d ',' -f1`
  ses=`echo $subject | cut -d ',' -f2`
  FS_Data=/dfs2/yassalab/rjirsara/ConteCenter/freesurfer/Conte-Two/${sub}_tp${ses}/stats/aseg.stats
  ...
  if [ -f ${FS_Data} ] ; then
    ...
    if [ ! -z "$jobstats" ]; then
       ...
       JobName=`echo F${sub}x${ses}`
       Pipeline=/dfs2/yassalab/rjirsara/ConteCenter/ConteCenterScripts/freesurfer/Conte-Two/freesurfer_anatomical_pipeline.sh
